...
-(void)match:(NSString*)source
{
    ...
    if(self.urlLink){
        [self matchUrlLink:source attrString:attStr offset:0 link:nil];
    }
    if(self.phoneLink){
        [self matchPhoneLink:source attrString:attStr offset:0 link:nil];
    }
    if(self.mobieLink){
        [self matchMobieLink:source attrString:attStr offset:0 link:nil];
    }
...
-(void)match:(NSString *)source   atCallBack:(BOOL (^)(NSString *))atString   title:(NSAttributedString *)title1   link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    _links=[[NSMutableArray alloc]init];
    if(self.urlLink){
        [self matchUrlLink:source attrString:attStr offset:offset link:link];
    }
    if(self.phoneLink){
        [self matchPhoneLink:source attrString:attStr offset:offset link:link];
    }
    if(self.mobieLink){
        [self matchMobieLink:source attrString:attStr offset:offset link:link];
    }
    CTParagraphStyleSetting lineBreakMode;
    CTLineBreakMode lineBreak = kCTLineBreakByCharWrapping;
    lineBreakMode.spec = kCTParagraphStyleSpecifierLineBreakMode;
...
{
    NSRegularExpression*regular=[[NSRegularExpression alloc]initWithPattern:@"^(\\(86\\))?(13[0-9]|15[7-9]|152|153|156|18[7-9])\\d{8}" options:NSRegularExpressionDotMatchesLineSeparators|NSRegularExpressionCaseInsensitive error:nil];
    NSArray* array=[regular matchesInString:source options:0 range:NSMakeRange(0, [source length])];
    for( NSTextCheckingResult * result in array){
        if(link==nil){
            CTFontRef fontRef=CTFontCreateWithName((__bridge CFStringRef)(self.font.fontName),self.font.pointSize,NULL);
...
{
    NSRegularExpression*regular=[[NSRegularExpression alloc]initWithPattern:@"^(\(\\d{3,4}\\)|\\d{3,4}-|\\s)?\\d{8}" options:NSRegularExpressionDotMatchesLineSeparators|NSRegularExpressionCaseInsensitive error:nil];
    NSArray* array=[regular matchesInString:source options:0 range:NSMakeRange(0, [source length])];
    for( NSTextCheckingResult * result in array){
        if(link==nil){
            CTFontRef fontRef=CTFontCreateWithName((__bridge CFStringRef)(self.font.fontName),self.font.pointSize,NULL);
